...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      highlightDate(day: Number, month: number, year: Number) {
        const date = `${year}-${this.prependZeroToDate(
          month
        )}-${this.prependZeroToDate(day)}`;
        console.log(date);
      ...
      handleEvents(events) {
        if (this.sidebarClickEvents.length === 0) this.removeHighlight();
        // console.log({ events });
