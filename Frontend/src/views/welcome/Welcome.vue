<template>
  <div class="welcome full-page centred">
    <div class="form-card centred">
      ...
      <a class="inline-link" @click="toggle">{{ infoText }}</a>
    </div>
  </div>
</template>
...
<script lang="ts">
  ...
  type Form = typeof LoginForm | typeof SignupForm;
  type FormTab = Form & DefineComponent;
  type FormComponent = HTMLElement & { activate: Function };
  ...
  export default defineComponent({
    ...
    data() {
      return {
        activeTab: this.$refs.login as FormTab,
      };
    },
    computed: {
      title() {
    ...
    methods: {
      ...
      activate() {
        ...
        const activeForm: FormComponent =
          this.activeTab === this.$refs.login
