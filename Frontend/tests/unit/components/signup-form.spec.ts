import { FULL_USER_REGISTER_ENDPOINT } from "@/services/endpoints";
...
import { mockApi, mockRouter } from "../mocks";

describe("Signup Form", () => {
  beforeAll(() => {
    mockApi
      .onPost(FULL_USER_REGISTER_ENDPOINT, {
        username: validRegisterUsername,
  ...
  it("No data entered", (done) => {
    testRegister(
      ...
      "",
      false,
      "Sign up failed. Please ensure no fields are left blank."
    ).then(() => {
      done();
    });
  });
  ...
  it("Short username (< 3 chars)", (done) => {
    testRegister(
      ...
      validRegisterPassword,
      false,
      "Sign up failed. Your username must be at least three characters long."
    ).then(() => {
      done();
    });
  });
  ...
  it("Short password (< 5 chars)", (done) => {
    testRegister(
      ...
      validRegisterPassword,
      false,
      "Sign up failed. Your password must be at least five characters long."
    ).then(() => {
      done();
    });
  });
  ...
  it("Non-matching password repeat", (done) => {
    testRegister(
      ...
      "other",
      false,
      "Sign up failed. The two entered passwords do not match."
    ).then(() => {
      done();
    });
  });
